...
pub(in crate::evaluator::builtins) fn single_ident_param(
    ...
    if text.is_empty() {
        return Err(EvalError::InvalidUsage(None, format!("{desc} cannot be empty")));
    }
...
mod tests {
    ...
    fn single_ident_param_rejects_non_param_and_empty_identifier_text() {
        ...
        let ident = node(
            NodeKind::Ident,
            src,
            token(src, TokenKind::Ident, 99, 1),
            vec![],
        );
        let param = node(NodeKind::Param, src, Token::synthetic(src, 0), vec![ident]);
        let text = node(
            NodeKind::Text,
            src,
            token(src, TokenKind::Text, 0, 1),
            vec![],
        );

        assert!(
            single_ident_param(&eval, &text, "name")
                .unwrap_err()
                .to_string()
                .contains("Param node")
        );
        assert!(
            single_ident_param(&eval, &param, "name")
                .unwrap_err()
                .to_string()
